...
const views = {
    ...
    <div class="mt-8 bg-[#050b1d] border border-white/5 p-8 rounded-[2.5rem] relative overflow-hidden">
        ...
        <div id="nebula-grid" class="flex flex-wrap gap-1.5 opacity-80">
            </div>
        <p class="text-[8px] text-gray-600 font-bold uppercase tracking-widest italic mt-4">
            Brighter squares indicate heavy coding days
        </p>
    </div>
    ...
    <div class="space-y-6 animate-in">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            <div class="bg-[#050b1d] border border-white/5 p-6 rounded-[2rem] relative overflow-hidden group hover:border-green-500/30 transition-all">
                <div class="flex items-center gap-4 relative z-10">
                    ...
                    <div>
                        <p class="text-[10px] font-black text-gray-500 uppercase tracking-widest">Current Level</p>
            ...
            <div class="bg-[#050b1d] border border-white/5 p-6 rounded-[2rem] relative overflow-hidden group hover:border-blue-400/30 transition-all">
                <div class="flex items-center gap-4 relative z-10">
                    ...
                    <div>
                        <p class="text-[10px] font-black text-gray-500 uppercase tracking-widest">Active Semester</p>
        ...
        <div id="lesson-sub-content">
            <div class="bg-[#050b1d] border border-white/5 p-12 rounded-[2.5rem] text-center">
                ...
                <h3 class="text-2xl font-black text-white uppercase tracking-tighter mb-4 italic">Ready to advance?</h3>
                <p class="text-gray-500 text-[10px] font-bold uppercase tracking-widest mb-8 max-w-xs mx-auto leading-relaxed">
                    Unlock your potential. Begin your fundamental course to master HTML, CSS, and modern web development.
                </p>
                <button onclick="startBeginnerCourse()" class="px-12 py-5 bg-white text-black rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] hover:bg-blue-600 hover:text-white transition-all transform hover:scale-105 active:scale-95 shadow-xl shadow-white/5">
                    Begin Journey
                </button>
            </div>
        </div>
...
// HELPER: Generate Activity Grid for Overview
function initActivityGrid() {
    const grid = document.getElementById('nebula-grid');
    if (!grid) return;
    
    grid.innerHTML = '';
    for (let i = 0; i < 154; i++) { // approx 22 weeks
        const square = document.createElement('div');
        square.className = 'w-[10px] h-[10px] rounded-[2px] bg-white/[0.03] transition-colors duration-500';
        
        // Randomly highlight to mimic the screenshot
        if (Math.random() > 0.85) {
            square.classList.replace('bg-white/[0.03]', 'bg-blue-500');
            square.style.boxShadow = '0 0 8px rgba(59, 130, 246, 0.4)';
        } else if (Math.random() > 0.95) {
            square.classList.replace('bg-white/[0.03]', 'bg-purple-500');
        }
...
function showLabAlert(feature) {
 ...
 //// for the nXXT LAB 2
const labModules = {
    'Sandbox': { icon: 'fa-terminal', color: 'text-blue-500', desc: 'Secure environment for code execution.' },
    'IDE v1': { icon: 'fa-code-branch', color: 'text-emerald-500', desc: 'Integrated workspace for project architecture.' },
    'API Terminal': { icon: 'fa-microchip', color: 'text-purple-500', desc: 'Direct neural endpoint telemetry.' },
    'UI Kit': { icon: 'fa-layer-group', color: 'text-pink-500', desc: 'Atomic design system and asset library.' },
    'Neural Playground': { icon: 'fa-brain', color: 'text-orange-500', desc: 'Cognitive bias and weight adjustment.' },
    'Log Stream': { icon: 'fa-stream', color: 'text-cyan-500', desc: 'Real-time kernel event monitoring.' }
};

function switchLabTab(tabName) {
    const container = document.getElementById('lab-content-area');
    const data = labModules[tabName];
    ...
    setTimeout(() => {
        container.innerHTML = `
            <div class="animate-in fade-in zoom-in-95 duration-500">
                <div class="border-2 border-dashed border-white/5 rounded-[3rem] py-20 flex flex-col items-center justify-center text-center bg-[#050505] relative overflow-hidden">
                    <div class="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-blue-600/5 via-transparent to-transparent opacity-50"></div>
                    
                    <div class="relative z-10 px-6">
                        <div class="w-20 h-20 bg-white/5 rounded-2xl flex items-center justify-center mb-6 mx-auto border border-white/10 shadow-2xl">
                            <i class="fas ${data.icon} text-3xl ${data.color} animate-pulse"></i>
                        </div>
                        <h4 class="text-white font-black uppercase italic tracking-tighter text-2xl">${tabName} Module</h4>
                        <p class="text-[10px] text-gray-500 font-bold uppercase tracking-[0.3em] mt-3 max-w-[320px] mx-auto leading-relaxed">
                            ${data.desc}<br><span class="text-yellow-500/80">Kernel stabilization in progress.</span>
                        </p>
                        
                        <div class="mt-8 flex justify-center gap-1 opacity-20">
                            ${[1,2,3,4,5].map(() => `<div class="w-6 h-1 bg-white/20 rounded-full"></div>`).join('')}
                        </div>
                    </div>
                </div>
            </div>
    ...
    const activeBtn = document.getElementById(`nav-${tabName.replace(/\s/g, '')}`);
