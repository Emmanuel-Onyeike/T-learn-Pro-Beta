 ...
 * Status: Integrated with Gemini 1.5 Flash API
 */
...
(function initNxxtSystem() {
    ...
    async function sendMessage() {
        ...
        try {
            // THE NEURAL FETCH (Corrected Endpoint)
            ...
            renderAiResponse(`CRITICAL ERROR: ${err.message}. Ensure API key is valid.`, 'text');
        }
    }
    ...
    function renderUserMessage(text) {
        document.getElementById('aiThread').insertAdjacentHTML('beforeend', `
            <div class="flex justify-end mb-8 animate-in fade-in slide-in-from-right-4 duration-300">
                <div class="max-w-[85%] bg-blue-600 text-white px-6 py-4 rounded-[2rem] rounded-tr-md shadow-xl shadow-blue-900/10">
    ...
    window.switchMode = function(mode) {
        ...
        if(mode === 'fun') {
            f.className = "px-4 py-1.5 rounded-lg text-[9px] font-black uppercase text-white bg-blue-600";
            s.className = "px-4 py-1.5 rounded-lg text-[9px] font-black uppercase text-white/30";
        } else {
            s.className = "px-4 py-1.5 rounded-lg text-[9px] font-black uppercase text-white bg-blue-600";
