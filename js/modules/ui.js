            ...
            <div class="space-y-8 animate-in">
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div class="space-y-2">
                        ...
                        <input type="password" id="currentPassword" placeholder="••••••••••••" class="settings-input">
                    </div>
                    <div class="space-y-2">
                        <label class="text-[9px] font-black text-gray-500 uppercase ml-2">New Password</label>
                        <input type="password" id="newPassword" placeholder="Enter new password (min 8 chars)" class="settings-input">
                    </div>
                </div>
                ...
                <button id="updatePasswordBtn" onclick="updatePassword()" class="w-full md:w-auto px-12 py-4 bg-blue-600 border border-blue-500/30 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all">Update Security</button>
            </div>`,
...
`
    ...
    // FIX: Render projects list when opening Settings > Projects tab
    if (tabName === 'Projects') {
        setTimeout(renderSettingsProjects, 50);
    }

    // FIX: Re-render history when opening History tab
    if (tabName === 'History') {
        setTimeout(() => {
    ...
    // FIX: Sync profile data when opening Profile tab
    if (tabName === 'Profile') {
        setTimeout(() => {
