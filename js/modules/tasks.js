...
window.switchTaskTab = function(tab) {
    ...
    tabs.forEach(t => {
        ...
        if (el) {
            el.className = "px-6 py-2.5 rounded-full border border-white/10 text-white/40 text-[10px] font-black uppercase tracking-widest transition-all";
        }
    });

    // 2. Set the clicked tab to active (Lime Green style)
    const activeBtn = document.getElementById('tab-' + tab);
    if (activeBtn) {
        activeBtn.className = "px-6 py-2.5 rounded-full bg-[#CCFF00] text-black text-[10px] font-black uppercase tracking-widest shadow-lg shadow-[#CCFF00]/10 transition-all";
    }

    // 3. Update the content area with the specific sector's empty state
    container.innerHTML = `
        <div class="animate-in fade-in zoom-in-95 duration-500 flex flex-col items-center justify-center text-center">
            <div class="w-20 h-20 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mb-6">
                <i class="fas fa-database text-white/10 text-3xl"></i>
            </div>
            ...
            <p class="text-white/20 text-[9px] font-bold uppercase tracking-[0.2em] max-w-xs leading-relaxed mx-auto">
                No data intercepted in this sector. Protocol is standing by for new task deployment.
            </p>
        </div>
    `;
};
...
window.toggleTimeView = function(view) {
    ...
    if (view === 'working') {
        btnW.className = "flex-1 py-2 rounded-full bg-[#CCFF00] text-black text-[10px] font-black uppercase tracking-widest transition-all";
        btnD.className = "flex-1 py-2 rounded-full text-white/40 text-[10px] font-black uppercase tracking-widest transition-all";
        // Logic to update chart to Working Data goes here
    } else {
        btnD.className = "flex-1 py-2 rounded-full bg-[#CCFF00] text-black text-[10px] font-black uppercase tracking-widest transition-all";
        btnW.className = "flex-1 py-2 rounded-full text-white/40 text-[10px] font-black uppercase tracking-widest transition-all";
        // Logic to update chart to Due Date Data goes here
    }
};
