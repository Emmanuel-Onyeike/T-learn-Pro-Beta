// dashboard.js - T Learn Pro Dashboard Logic
        ...
        <div class="max-w-md mx-auto content-card text-center animate-in">
            ...
            <div class="space-y-4 mb-8">
                <div class="p-4 bg-white/5 border border-white/5 rounded-2xl text-left">
                    ...
                    <p class="text-white text-[11px] font-bold">Your account is active. Start your first lesson today!</p>
                </div>
            </div>
        ...
        <div class="space-y-8 animate-in">
            <div class="space-y-6">
                ...
                <div class="relative group">
                    ...
                    <input type="text" placeholder="SEARCH REPOSITORY BY NAME, TAG, OR TECH STACK..." class="w-full bg-white/5 border border-white/10 rounded-2xl py-5 pl-14 pr-6 text-[10px] font-black text-white placeholder-gray-700 uppercase tracking-widest outline-none focus:border-blue-500/50 focus:bg-blue-500/5 transition-all">
                </div>
...
function updateSettingsTab(tabName) {
    ...
    document.querySelector(`button[onclick="updateSettingsTab('${tabName}')"]`)?.classList.add('active');

    container.innerHTML = settingsTabs[tabName] || `<div class="py-20 text-center text-gray-500 font-black uppercase text-[10px] tracking-[0.2em] italic">${tabName} module is under construction</div>`;

    // Load toggle states if on Notif-Settings
    if (tabName === 'Notif-Settings') {
        setTimeout(loadToggleStates, 50);
    }
}

// Toggle switches with localStorage
function toggleSwitch(id) {
    const btn = document.getElementById(id);
    const isOn = btn.classList.toggle('on');
    localStorage.setItem(id, isOn);
}
